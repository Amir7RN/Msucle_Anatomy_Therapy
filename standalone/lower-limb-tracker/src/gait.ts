...
/**
 * Ankle angle for one leg = depth-trust-capped blend of the image-plane and
 * world-space vertex angle at the ankle (KNEE-ANKLE-TOE), complemented to
 * preserve the shank-line/foot-line clinician convention (see the module
 * docstring — the `180 -` is not optional). At neutral standing this is
 * ~70–95°; it INCREASES with dorsiflexion (toe lifts toward shin) and
 * DECREASES with plantarflexion (toe drops, push-off). Returns null if any
 * of the three joints is missing.
 */
function ankleVertexAngle(lms: LandmarkSet, knee: number, ankle: number, toe: number): number | null {
  const k = lms[knee], a = lms[ankle], t = lms[toe]
  if (!k || !a || !t) return null
  const dr = Math.min(ANKLE_DEPTH_TRUST_CAP, depthReliability(lms, knee, ankle, toe))
  return 180 - adaptiveVertexAngle(k, a, t, dr)
}
...
export function measureGaitFrame(lms: LandmarkSet): GaitFrameMetrics {
  ...
  const leftAnkle  = lVis >= FOOT_MIN_VIS ? ankleVertexAngle(lms, LM.L_KNEE, LM.L_ANKLE, LM.L_FOOT_IDX) : null
  const rightAnkle = rVis >= FOOT_MIN_VIS ? ankleVertexAngle(lms, LM.R_KNEE, LM.R_ANKLE, LM.R_FOOT_IDX) : null
...
// ─────────────────────────────────────────────────────────────────────────────
//  Gap-fill — bridge short tracking dropouts in a single scalar channel
//
...
//  blur) — extrapolating a full-length gap "as if it keeps going" is most
//  likely to be wrong at the moment the real signal is about to turn around.
//  So: extrapolate briefly, then hold flat, then give up and report a genuine
//  null ("tracking lost") past MAX_GAP_MS — never fabricate data indefinitely
//  through a real occlusion.
// ─────────────────────────────────────────────────────────────────────────────
...
export class GaitGapFiller {
  ...
  private vel = 0   // deg/s, lightly smoothed

  push(raw: number | null, tMs: number): GapFillResult {
    if (raw != null) {
      if (this.lastGood != null && this.lastGoodT != null) {
        const dt = (tMs - this.lastGoodT) / 1000
        if (dt > 1e-3) this.vel = 0.5 * ((raw - this.lastGood) / dt) + 0.5 * this.vel
      }
      this.lastGood = raw
      this.lastGoodT = tMs
      return { value: raw, interpolated: false }
    }
    if (this.lastGood == null || this.lastGoodT == null) return { value: null, interpolated: false }
    const gapMs = tMs - this.lastGoodT
    if (gapMs > GaitGapFiller.MAX_GAP_MS) return { value: null, interpolated: false }
    const extrapMs = Math.min(gapMs, GaitGapFiller.VELOCITY_MS)
    const drift = clampNum(this.vel * (extrapMs / 1000), -GaitGapFiller.MAX_EXTRAPOLATION_DEG, GaitGapFiller.MAX_EXTRAPOLATION_DEG)
    return { value: this.lastGood + drift, interpolated: true }
  }
