 ...
 * pipeline.  Four layers that every serious markerless-mocap stack uses:
 *
 ...
 *
 *   2. Per-user, per-channel ADAPTIVE weighting
...
export function createLandmarkFilter(): LandmarkFilter {
  ...
  function push(raw: LandmarkSet, tMs: number): LandmarkSet {
    ...
    // a run of still, in-frame frames. Motion resets the counter so we don't
    // declare convergence mid-rep.
